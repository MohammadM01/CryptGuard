...
start "CryptGuard DPI Engine" cmd /k "cd Packet_analyzer && echo Building Engine... && g++ -std=c++17 -pthread -O2 -I include -o dpi_engine.exe src/dpi_mt.cpp src/pcap_reader.cpp src/packet_parser.cpp src/sni_extractor.cpp src/types.cpp && echo Generating test traffic... && python generate_test_pcap.py && echo Running Engine... && dpi_engine.exe test_dpi.pcap output.pcap"
